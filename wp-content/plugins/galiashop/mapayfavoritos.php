...
//		echo '<br><br>';
		...
		*/
		...
		echo '<div id="carrufavoritos">';

		if ( function_exists( 'get_sites' ) && class_exists( 'WP_Site_Query' ) ) {
		    $sites = get_sites(array('orderby'=>'id','order'=>'DESC'));
		    foreach ( $sites as $site ) {
		    	$listafavoritos = get_user_favorites(null, $site->blog_id, null);
		...
		*/
		...
		if ( function_exists( 'get_sites' ) && class_exists( 'WP_Site_Query' ) ) {
		    ...
		    foreach ( $sites as $site ) {
		        ...
		        $configuracion = get_option( 'blog_globalid' ); 
		        echo '<div class="favoproduc tiendavi my-inline-block-class">';
					...
					//echo '<img class="imgap" src="'.$configuracion['tfoto'].'" style="height:100px; width:auto;">';
					 echo '<img  src="'.plugin_dir_url( __FILE__ ).'/tienda.png" style="height:100px; width:auto;">';
...
//		echo '<br><br>';
		...
		echo '<div class="posicarrou listadogral">';
		// run the loop
...
<div id="cargador">
					...
					<form class="container-fluid" id="formbuscador"  action="<?php bloginfo('url') ?>" method="GET">
						
						<div class="row">
							<div class="col-md-6 campo1">
						<input type="text" style="width: 87%; " id="interbusca" placeholder="Producto o tienda a buscar" name="s" /> <span style="color:white;font-size: 20px;" class="glyphicon glyphicon-search"></span>
				
							</div>
							<div class="col-md-6 campo2" >
						<div id="slider"></div>
						
							</div>
							<div class="col-md-6 campo3" >
								<input type="number" name="pdesde" id="pdesde" placeholder="Desde Gs" style="width:40%" />
						<input type="number" name="phasta" id="phasta" placeholder="Hasta Gs" style="width:53%" />	
...
echo '</div>'; //cierre de contemapa
		echo '<div id="resultadobus" ></div>';
		echo '<div id="resultadotiendas" ></div>';
...
function my_styles_method() {
			wp_enqueue_style(
		        ...
		        .marcafavo{
					...
					#resultadotiendas .owl-item h3{
					color:#f3f3f3;
				}

				#resultadotiendas::before{
					content: 'Tiendas Encontradas';
				    font-weight: bold;
				    font-size:26px;
				    color:#f3f3f3;
				    padding-top:15px;
				    padding-bottom:20px;
						...
						.campo1{
							width: 30%;  display: inline-block; vertical-align: top; margin-right: 3%;
						}
						.campo2{
						...
						@media (max-width: 600px) {
							...
							#mapapers{
								height:550px !important; /*250 estaba*/
							}
							#formbuscador{
								position:relative;
								margin-top:14px;
...
function google_maps_script_loader() {
    ...
    if ($gmapsenqueued) {
        ...
        wp_enqueue_script('google-maps', 'https://maps.googleapis.com/maps/api/js?key='.$configuracion["tgapi"].'&libraries=places', array('jquery'), false, false);
    }
}
...
function favocarousel(){
	...
	$codigo = '
	  ...
	  //if(checkWidth <=600){
	    ...
	    jQuery("#carrufavoritos").owlCarousel({
	        autoplay: true,
	        pagination: true,
	        navigation: true,
	        navigationText: ["Anterior", "Siguiente"],
	    ...
	    var leftm = 0;
	    var elementoselec;
	    var hacerefecto = false;
	    var elementoeliminar;
	    var defavoritos = false;
...
*/
		...
		//actualiza al eliminar favoritos
		jQuery("#carrufavoritos").on("click", ".simplefavorite-button", function() {
			
		  		elementoeliminar = jQuery(this).parent().parent();
		  		defavoritos = true;
        }); 
		...
		jQuery(".listadogral").on("click", ".simplefavorite-button", function() {
			//alert("se ha");
                var offset = jQuery(this).offset();
		  		topm = offset.top;
		  		leftm = offset.left; 

		  		elementoselec = jQuery(this).parent().parent().clone();
		  		hacerefecto = true;

            }); 

        jQuery("#resultadobus").on("click", ".simplefavorite-button", function() {
        	//alert("se ha2");
                var offset = jQuery(this).offset();
		  		topm = offset.top;
		  		leftm = offset.left;
	    ...
	    jQuery(document).on("favorites-updated-single", function(event, favorites, post_id, site_id, status){
		    //alert(status);
		    if(status=="active"  && hacerefecto ==true){

		    	//alert("se ha agregado un favorito" + post_id);
			    //AppentMouse();
			    jQuery( "body" ).append( "<div class=\"marcafavo\"><img src=\"'.plugin_dir_url( __FILE__ ).'/estrella.png\"></div>");

			    jQuery(".marcafavo").css("top", topm);
			    jQuery(".marcafavo").css("left", leftm);

			    //jQuery(".marcafavo").fadeDown();
			    //jQuery(".marcafavo").remove();
			    //alert("2");

			    var topfav = jQuery(".widget_listadofav_widget").offset().top;

			      jQuery(".marcafavo").animate({
				    "top": topfav+"px"
				    //"marginTop" : "+=540px" //moves down
				  }, 500, "linear", function() {
				    	jQuery(".marcafavo").remove();
				  });


				jQuery("#carrufavoritos").owlCarousel().data("owlCarousel").destroy();

			  	//jQuery("#carrufavoritos").prepend( "<div ><img src=\"'.plugin_dir_url( __FILE__ ).'/estrella.png\"></div>");
				jQuery("#carrufavoritos").prepend(elementoselec);
		    ...
		    if(status=="inactive"  && defavoritos == true){
    ...
    /*jQuery( "#interbusca" ).autocomplete({
	    ...
	    $codigo3 = '
	     jQuery(document).ready(function () {
	     	...
	     	jQuery("#resultadobus").hide();
			jQuery("#resultadotiendas").hide();
	    	...
	    	jQuery( "#interbusca" ).on("input", function() {
	    		if(jQuery( "#interbusca" ).val().length > 2){
	    			buscarajax();
	    		}
			 	...
			 	jQuery.ajax({
				      ...
				      success: function(msg){
						 ...
						 if(msg.data !== null && msg.data !== "") {
						 	...
						 	jQuery("#resultadobus").html(  msg.html); 
					           ...
					           for (var i = 0; i < msg.data.length; i++) {
									...
									//});
					          		...
					          		var datai = "<h3>" + msg.data[i].nombretienda + "</h3>" + "<br>" + msg.data[i].tfoto + "<br>"+ msg.data[i].productos;
							        ...
							        loc2 = new google.maps.LatLng(parseFloat(msg.data[i].latitud), parseFloat(msg.data[i].longitud));
						  			bounds2.extend(loc2);
									...
									tiendashtml = tiendashtml + "<div>"+datai+"</div>";
					          ...
					          mapapers.fitBounds(bounds2);

					          //para que no acerque demasiado el zoom cuando es 1
					          var zoom = mapapers.getZoom();
							  mapapers.setZoom(zoom > 15 ? 15 : zoom);

						
								  jQuery("#resultadobus").show();
								  jQuery("#resultadotiendas").show();

						          jQuery("#resultadotiendas").html(tiendashtml);
							 ...
							 //jQuery("#resultadobus").owlCarousel("update");

							


					        /*   var owl = jQuery("#resultadobus").data("owlCarousel");
					         ...
					         jQuery("#resultadobus").owlCarousel({
							        autoplay: true,
							        pagination: true,
							        navigation: true,
							        navigationText: ["Anterior", "Siguiente"]
					          ...
					          jQuery("#cargador").hide();
					          jQuery("#resultadotiendas").owlCarousel().data("owlCarousel").destroy();
					      	jQuery("#resultadobus").owlCarousel().data("owlCarousel").destroy();
			                ...
			                navigation: true
					     	...
					     	//alert("no se ha enc");
					     	jQuery("#cargador").hide();
					     	jQuery("#resultadotiendas").html("<span style=\"color:white\"><br>No se han encontrado productos</span>");
					     	jQuery("#resultadobus").html("<span style=\"color:white\"><br>No se han encontrado tiendas</span>"); 
					     }
				      ...
				      complete: function(msg){
				      	...
				      	if(msg.data !== null && msg.data !== "") {
					      	jQuery("#resultadotiendas").owlCarousel({
				                items: 5,
				                navigation: true
				            });
...
/*
    ...
    public function tcolor_callback()
    {
    	$resultado = $this->options['tcolor'];
    	echo '<input type="text" id="tcolor" name="blog_globalid[tcolor]" value="'. $resultado  .'"  >';
    }
	    ...
	    foreach ( $sites as $site ) {
	    	...
	    	if(in_array($site->blog_id, $tiendas)){
		        ...
		        if(sizeof($network_q_posts) > 0){
		        	...
		        	$arraycillo['tfoto'] = '<img  src="'.plugin_dir_url( __FILE__ ).'/tienda.png" style="height:100px; width:auto;">';
		        	if($configuracion['tdesc'] != ""){
							//$test = json_encode($configuracion['tdesc']);
							$test = esc_js($configuracion['tdesc']);
							$test = str_replace('\n\n', '<br>', $test);
