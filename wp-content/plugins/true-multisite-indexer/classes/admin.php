...
if ( !class_exists( 'multisiteindexerAdmin' ) ) {
	...
	class multisiteindexerAdmin {
		...
		function __construct() {
			...
			/*
			 * Эти хуки по удалению сайта из индекса запускаются, когда мы архивируем/удаляем и т д блоги
			 */
		...
		function process_admin_page() {
			...
			switch( $_POST['action'] ) {
				...
				case 'true_multisite_globaloptions':
					...
					update_site_option( 'hide_indexed_posts_page', $_POST['hide_indexed_posts_page'] );
				...
				case 'true_multisite_license' :
					...
					delete_transient( 'misha_upgrade_mul470e2_9t8t' );
					wp_safe_redirect( add_query_arg( array( 'saved' => 1 ), wp_get_referer() ) );
					exit;
					break;
			}
		...
		/*
		 * Функция, удаляющая целый блог из индекса
		 */
