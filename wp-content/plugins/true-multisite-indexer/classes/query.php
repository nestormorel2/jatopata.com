...
class Network_Query {
	...
	*/
	...
	function &get_posts() {
		...
		// [rudrastyh upd.1.05] blog can be passed as array
		if( !empty( $q['blog_id'] ) ) {
			if( !is_int( $q['blog_id'] ) ) {
				$blog_id_array = $q['blog_id'];
				if( !is_array( $blog_id_array ) )
					$blog_id_array = preg_split('/[,\s]+/', $q['blog_id']);

				$operator = 'AND (';
				foreach( $blog_id_array as $blog_id ) {
					$where .= " " . $operator . " ($this->network_posts.BLOG_ID = " . absint($blog_id) . ')';
					$operator = 'OR';
				}
				$where .= ')';
			} else {
...
require_once( dirname(__FILE__) . '/meta-query.php');
// /**
