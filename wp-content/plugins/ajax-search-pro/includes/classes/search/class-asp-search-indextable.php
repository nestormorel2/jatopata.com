...
if (!class_exists('ASP_Search_INDEX')) {
	...
	class ASP_Search_INDEX extends ASP_Search_CPT {
		...
		protected function do_search() {
			...
			/*----------------------- Optimal LIMIT -------------------------*/
			/**
			 * Instead of the flippy-floppy calculations, just set this to 20k
			...
			if ( in_array('title', $args['post_fields']) ) {
				...
				SELECT
					...
					$custom_field_selectp as customfp,
                	$custom_field_selects as customfs,
					IF($wpdb->posts.post_title LIKE '$s',
						(" . $rel_val_title * 80 . "),
						(" . $rel_val_title * 40 . ")
					)as relevance";

				// For exact matching logis we need exact title, or title starting and continuing
				if ( $kw_logic == 'orex' || $kw_logic == 'andex' ) {
					$_rep = "($wpdb->posts.post_title LIKE '$s' || $wpdb->posts.post_title LIKE '$s %')";
				} else {
					$_rep = "($wpdb->posts.post_title LIKE '$s%')";
				}
